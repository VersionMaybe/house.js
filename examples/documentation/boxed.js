export const Boxed = (function(options){
    ...
    this.navigation = {
        ...
        go: (path, replaceState) => {
            ...
            let details = this.navigation.exists(path);
            if (!details) {
                warning(`Path "${path}" does not exist or has not been setup.`);

                if (this.navigation.fallback == null) {
                    error(`No navigation fallback has been specified for pages that don't exist!`);
                    return;
                }
                if (typeof this.navigation.fallback === 'number') {
                    details = this.navigation.paths[this.navigation.fallback];
                } else {
                    details = { component: 'page-error' };
                }
    ...
    function init(app, { components, services, updateFreqency, navigation, settings }) {
        ...
        if (navigation) {
            ...
            app.navigation.fallback = navigation.fallback;
            ...
            if (typeof navigation.fallback === 'string') {
                loadComponent(app, {
                    name: 'error',
                    component: navigation.fallback
                }, 'page');
            }
        }
        ...
        (services || []).forEach(element => {
            loadComponent(app, element, 'service', true);
        });
    ...
    async function loadComponent(app, { name, component }, prefix = 'app', scriptOnly = false) {
        ...
        if (!customElements.get(`${prefix}-${name}`)) {
            ...
            if (scriptOnly) {
                ...
                if (componentJS.error) { error(`Service '${ name }' requires a .js file.`); }
                
                initialiseComponent({
                    app,
                    prefix, name,
                    template: null, templateJS, templateStyle: null, path: component + `/${ dirName }.js`
                });

                // document.body.parentElement.insertBefore(document.createElement(`${prefix}-${name}`), document.body);
                document.body.parentElement.appendChild(document.createElement(`${prefix}-${name}`));
                // document.body.appendChild(document.createElement(`${prefix}-${name}`));
            } else {
                // Load Component or page
                ...
                initialiseComponent({
                    app,
                    prefix, name,
                    template, templateJS, templateStyle, path: component + `/${ dirName }.js`
                });
            }
        } else {
    ...
    function initialiseComponent({app, prefix, name, template, templateJS, templateStyle, path}) {
        customElements.define(`${prefix}-${name}`, class extends HTMLBoxedElement {
            ...
            setup() {
                ...
                try {
                    ...
                    error(`An error within a component's javascript occurred.\nComponent: ${name}\nPath: ${path}\nType: ${e.name}\nMessage: ${e.message}`)
                    // Run again so that dev-tools shows the error and line number for debugging help.
                    templateLogic({ app, content: this.boxedContent, instance: this });
                }
